[xml]$XmlDocument = Get-Content -Path "C:\Users\martijn\Downloads\Collectivo Medellin\Rutas_de_Transporte_Público_Colectivo.kml" -Encoding UTF8
...
foreach ($AgencyRow in $AgencyArray) {
    ...
    agency_url = 'https://www.planner.colombiainfo.org'
...
$Agencies | Export-Csv -Append -Path "C:\Temp\agency.txt" -NoTypeInformation -Encoding UTF8 -Delimiter "," | % {$_ -replace '"',''} 
...
foreach ($Placemark in $XmlDocument.kml.Document.Folder.Placemark) {  
   ...
   $shapes += $shape
    ...
    $shapes | Export-Csv -Append -Path "C:\Temp\shapes.txt" -NoTypeInformation -Encoding UTF8 -Delimiter "," | % {$_ -replace '"',''}
    $route | Export-Csv -Append -Path "C:\Temp\routes.txt" -NoTypeInformation -Encoding UTF8 -Delimiter "," | % {$_ -replace '"',''} 
    $trip | Export-Csv -Append -Path "C:\Temp\trips.txt" -NoTypeInformation -Encoding UTF8 -Delimiter "," | % {$_ -replace '"',''} 
}
