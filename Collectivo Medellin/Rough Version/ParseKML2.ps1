$path = "C:\Users\martijn\Downloads\Collectivo Medellin\Paradas_de_Transporte_Público_Colectivo.csv"
...
$stops | Export-Csv -Path "C:\Temp\stops.txt" -NoTypeInformation -Encoding UTF8 -Delimiter "," | % {$_ -replace '"',''}
...
Foreach ($UniqueRuta in $UniqueRutas) {
    ...
    $stop_times | Export-Csv -Append -Path "C:\Temp\stop_times.txt" -NoTypeInformation -Encoding UTF8 -Delimiter "," | % {$_ -replace '"',''}
